...
<div>
		...
		<table>
			<tr>
				<td> <h2>Username:</h2> </td> <td> <input type="text" v-model="user.username" name="username"/> </td>
			</tr>
			<tr>
			...
			<tr>
				<td> <h2>Prezime:</h2> </td> <td> <input type="text" v-model="user.prezime" name="prezime"/> </td>
			</tr>
			<tr>
				<td> <h2>Email:</h2> </td> <td> <input type="text" v-model="user.email" name="email"/> </td>
			</tr>
			<tr>
				<td> <h2>Broj telefona:</h2> </td> <td> <input type="text" v-model="user.telefon" name="telefon"/> </td>
			</tr>
			<tr>
...
`,
    methods: {
        registerUser: function() {
            ...
            axios.post("/api/users/updateSupp", newUser).then(data => {
                if (data.data == "OK") {
                    alert("Uspesno ste registrovali dobavljaca! Moze se ulogovati");
                }
            });
        },
    ...
    mounted() {
		...
		.then(function(resp){
			...
			self.currentPosition.lat = resp.data.lokacija.duzina;
			self.currentPosition.lon = resp.data.lokacija.sirina;
			self.currentPosition.adresa = resp.data.lokacija.ulica;	
			self.confPass = resp.data.noviPassw;
