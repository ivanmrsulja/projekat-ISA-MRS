Vue.component("register-adminphar", {
    data: function() {
        return {
...
<div>
		...
		<table>
			...
			<tr>
				<td> <h2>Apoteka:</h2> </td> <td> <select id="pharmas">
												  <option v-for="option in pharmacies" v-bind:value="option.id">
												    {{ option.naziv }}
												  </option>
												</select> 
				</td>
...
`,
    methods: {
        registerUser: function() {
            ...
            let newUser = { username: usr, noviPassw: pas, ime: ime, prezime: prz, email: email, telefon: tel, lokacija: lok, loggedBefore: false, apoteka: apo};
